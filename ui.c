...
#define COLOR_FOOD1			RGB(0, 255, 0)
#define COLOR_FOOD2			RGB(153, 0, 0)
...
void GamePaint(HWND hwnd)
{
	...
	*******************************************************************************/
	...
	hbrushFood1 = CreateSolidBrush(COLOR_FOOD1); // RGB颜色，实心BRUSH
	hbrushFood2 = CreateSolidBrush(COLOR_FOOD2); // RGB颜色，实心BRUSH
	hpen = CreatePen(PS_NULL, 0, RGB(0, 0, 0));  // PEN， PS_NULL表示不可见
	hpen1 = CreatePen(PS_NULL, 0, RGB(0, 0, 0));  // PEN， PS_NULL表示不可见
	hpen2 = CreatePen(PS_NULL, 0, RGB(0, 0, 0));  // PEN， PS_NULL表示不可见
	...
	*******************************************************************************/
	...
	Rectangle(hdcmem,
		lpFood1->x * CELL_PIXEL + rectBoundary.left,
		lpFood1->y * CELL_PIXEL + rectBoundary.top,
		(lpFood1->x + 1)*CELL_PIXEL + rectBoundary.left,
		(lpFood1->y + 1)*CELL_PIXEL + rectBoundary.top);

	hO2dBrush = (HBRUSH)SelectObject(hdcmem, hbrushFood1);
	hO2dPen = (HPEN)SelectObject(hdcmem, hpen1);
	...
	// （椭）圆形，使用上面选择的PEN勾勒边框，BRUSH填充
	Rectangle(hdcmem,
		lpFood2->x * CELL_PIXEL + rectBoundary.left,
		lpFood2->y * CELL_PIXEL + rectBoundary.top,
		(lpFood2->x + 1)*CELL_PIXEL + rectBoundary.left,
		(lpFood2->y + 1)*CELL_PIXEL + rectBoundary.top);

	hO3dBrush = (HBRUSH)SelectObject(hdcmem, hbrushFood2);
	hO3dPen = (HPEN)SelectObject(hdcmem, hpen2);
	...
	// （椭）圆形，使用上面选择的PEN勾勒边框，BRUSH填充
	Rectangle(hdcmem,
		lpFood3->x * CELL_PIXEL + rectBoundary.left,
		lpFood3->y * CELL_PIXEL + rectBoundary.top,
		(lpFood3->x + 1)*CELL_PIXEL + rectBoundary.left,
		(lpFood3->y + 1)*CELL_PIXEL + rectBoundary.top);
	...
	*******************************************************************************/
	...
	DeleteObject(hbrushFood1);
	DeleteObject(hbrushFood2);
